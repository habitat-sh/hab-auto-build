const fn = (async () => {
    ...
    const height = (Object.keys(data.nodes).length - 1) * step + margin.top + margin.bottom;
    ...
    const graph = (() => {
        ...
        for (const node_index in data.nodes) {
            let node_data = data.nodes[node_index];
            let node = {};
            switch (node_data.dependency_type) {
                case "local_plan":
                    node = {
                        id: `${node_data.data.id.origin}/${node_data.data.id.name}/${node_data.data.id.version}`,
                        sourceLinks: [],
                        targetLinks: [],
                        feedbackSourceLinks: [],
                        feedbackTargetLinks: [],
                    ...
                    break;
                case "remote_dependency":
                    node = {
                        id: `${node_data.data.origin}/${node_data.data.name}${node_data.data.version != null ? `/${node_data.data.version}` : ""}`,
                        sourceLinks: [],
                        targetLinks: [],
                        feedbackSourceLinks: [],
                        feedbackTargetLinks: [],
            ...
            nodes.push(node);
            nodeById.set(parseInt(node_index), node);
        }
    ...
    const chart = (() => {
        const header = d3.select("#graph-header").text(`${graph.nodes.length} Packages`);

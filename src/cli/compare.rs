...
pub(crate) fn execute(args: Params) -> Result<()> {
    ...
    let source_run_context = AutoBuildContext::new(&source_config, &source_config_path)
        .with_context(|| eyre!("Failed to initialize run"))?;
    let target_run_context = AutoBuildContext::new(&target_config, &target_config_path)
        .with_context(|| eyre!("Failed to initialize run"))?;
